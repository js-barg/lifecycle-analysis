...
class LifecycleExcelBuilder {
  ...
  /**
   * Sheet 4: Lifecycle Timeline (Year view)
   */
  async createLifecycleTimeline(workbook, products, options) {
    ...
    products.forEach(product => {
      const rowData = [
        ...
        product.description || '-',
        product.total_quantity,
        product.end_of_sale_date ? new Date(product.end_of_sale_date).getFullYear() : '',
        product.last_day_of_support_date ? new Date(product.last_day_of_support_date).getFullYear() : ''
      ];
      
      // Add year data
      const eosYear = product.end_of_sale_date ? new Date(product.end_of_sale_date).getFullYear() : null;
      const ldosYear = product.last_day_of_support_date ? new Date(product.last_day_of_support_date).getFullYear() : null;
      
      yearColumns.forEach(year => {
        const yearQty = product.year_quantities?.[year] || '';
        rowData.push(yearQty);
      });
      
      const row = sheet.addRow(rowData);
      
      // Apply conditional formatting to year cells
      yearColumns.forEach((year, index) => {
        const cell = row.getCell(8 + index); // Year columns start at column 8
    ...
    // Set column widths
    sheet.getColumn(1).width = 20; // Manufacturer
    sheet.getColumn(2).width = 20; // Category
    sheet.getColumn(3).width = 15; // Product ID
    sheet.getColumn(4).width = 40; // Description
    sheet.getColumn(5).width = 10; // Quantity
    sheet.getColumn(6).width = 10; // EOS Year
    sheet.getColumn(7).width = 10; // LDOS Year
    // Year columns
    for (let i = 8; i <= 7 + yearColumns.length; i++) {
